...
@onready var background_texture: ColorRect = get_node("%BackgroundTexture")
...
func _ready() -> void:
	if not graph_ui: return

	if graph_ui.camera_2d:
		init_background()
	else:
		graph_ui.camera_initialized.connect(_on_graph_ui_camera_initialized)


func _exit_tree() -> void:
	if graph_ui:
		if graph_ui.camera_initialized.is_connected(_on_graph_ui_camera_initialized):
			graph_ui.camera_initialized.disconnect(_on_graph_ui_camera_initialized)
...
func _on_graph_ui_camera_initialized() -> void:
	init_background()
